...
function TripDetail() {
  ...
  return <>
    <AdminTopbar title={route} subtitle="Detalhe da rota fixa do veiculo, paradas e despesas" actions={<><a href="/admin/viagens" className="rounded-md border border-input bg-card px-3 py-2 text-sm">← Voltar</a><ActionDialog triggerLabel="Editar viagem" title="Editar viagem" description="Atualize responsaveis, KM e status. A rota e editada no veiculo." submitLabel="Salvar viagem" triggerClassName="rounded-md border border-input bg-card px-3 py-2 text-sm hover:bg-accent" fields={[{ label: "Motorista", type: "select", value: d?.id, options: drivers.map((driver) => ({ label: driver.name, value: driver.id })) }, { label: "Veiculo / rota", type: "select", value: v?.id, options: vehicles.map((vehicle) => ({ label: `${vehicle.plate} - ${vehicle.fixedRoute}`, value: vehicle.id })) }, { label: "Rota fixa", type: "summary", value: route }, { label: "KM inicial", type: "number", value: t.initialKm }, { label: "KM final", type: "number", value: t.finalKm ?? "" }, { label: "Status", type: "select", value: t.status, options: Object.entries(tripStatusLabel).map(([value, label]) => ({ label, value })) }, { label: "Observacoes", type: "textarea", value: t.notes, wide: true }]} /></>} />

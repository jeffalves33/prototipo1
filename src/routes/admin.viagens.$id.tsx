...
function TripDetail() {
  ...
  return <>
    <AdminTopbar title={`${t.origin} → ${t.destination}`} subtitle="Detalhe da rota, veículo utilizado, paradas e despesas" actions={<><a href="/admin/viagens" className="rounded-md border border-input bg-card px-3 py-2 text-sm">← Voltar</a><ActionDialog triggerLabel="Editar viagem" title="Editar viagem" description="Atualize rota, responsáveis, KM e status." submitLabel="Salvar viagem" triggerClassName="rounded-md border border-input bg-card px-3 py-2 text-sm hover:bg-accent" fields={[{ label: "Motorista", type: "select", value: d?.id, options: drivers.map((driver) => ({ label: driver.name, value: driver.id })) }, { label: "Veículo", type: "select", value: v?.id, options: vehicles.map((vehicle) => ({ label: `${vehicle.plate} · ${vehicle.model}`, value: vehicle.id })) }, { label: "Origem", value: t.origin }, { label: "Destino", value: t.destination }, { label: "KM inicial", type: "number", value: t.initialKm }, { label: "KM final", type: "number", value: t.finalKm ?? "" }, { label: "Status", type: "select", value: t.status, options: Object.entries(tripStatusLabel).map(([value, label]) => ({ label, value })) }, { label: "Observações", type: "textarea", value: t.notes, wide: true }]} /></>} />

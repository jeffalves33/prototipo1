...
function TripDetail() {
  ...
  const route = v?.fixedRoute ?? `${t.origin} -> ${t.destination}`;
  const { origin, destination } = splitFixedRoute(route);

  return <>
    <AdminTopbar title={route} subtitle="Detalhe da rota fixa do veiculo, paradas e despesas" actions={<><a href="/admin/viagens" className="rounded-md border border-input bg-card px-3 py-2 text-sm">← Voltar</a><ActionDialog triggerLabel="Editar viagem" title="Editar viagem" description="Atualize responsaveis, datas, KM e status. A rota e editada no veiculo." submitLabel="Salvar viagem" triggerClassName="rounded-md border border-input bg-card px-3 py-2 text-sm hover:bg-accent" fields={[{ label: "Motorista", type: "select", value: d?.id, options: drivers.map((driver) => ({ label: driver.name, value: driver.id })) }, { label: "Veiculo / rota", type: "select", value: v?.id, options: vehicles.map((vehicle) => ({ label: `${vehicle.plate} - ${vehicle.fixedRoute}`, value: vehicle.id })) }, { label: "Rota fixa", type: "summary", value: route }, { label: "Data inicial", type: "date", value: t.startedAt.slice(0, 10) }, { label: "Data final", type: "date", value: t.finishedAt?.slice(0, 10) ?? "" }, { label: "KM inicial", type: "number", value: t.initialKm }, { label: "KM final", type: "number", value: t.finalKm ?? "" }, { label: "Status", type: "select", value: t.status, options: Object.entries(tripStatusLabel).map(([value, label]) => ({ label, value })) }, { label: "Observacoes", type: "textarea", value: t.notes, wide: true }]} /></>} />
    <div className="space-y-4 p-6">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <SectionCard title="Dados da viagem"><DetailRow label="Status" value={<StatusBadge tone={tripStatusTone[t.status]}>{tripStatusLabel[t.status]}</StatusBadge>} /><DetailRow label="Origem" value={origin} /><DetailRow label="Destino" value={destination} /><DetailRow label="Data inicial" value={formatDateTime(t.startedAt)} /><DetailRow label="Data final" value={formatDateTime(t.finishedAt)} /><DetailRow label="KM inicial" value={num(t.initialKm)} /><DetailRow label="KM final" value={t.finalKm ? num(t.finalKm) : "—"} /><DetailRow label="Observações" value={t.notes || "—"} /></SectionCard>
